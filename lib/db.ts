...
export async function getPaginatedProducts(
    ...
    try {
        ...
        // Apply filters
        if (filters?.categories && filters.categories.length > 0) {
             // If "Todos os Produtos" is in the list, we don't filter by category
             if (!filters.categories.includes("Todos os Produtos")) {
                 query = query.in('category', filters.categories);
             }
        }
...
export async function getProductNames(categories?: string[], search?: string): Promise<{ name: string }[]> {
    try {
        let query = supabase.from('products').select('name');
        
        if (categories && categories.length > 0 && !categories.includes("Todos os Produtos")) {
        ...
        // Limit to 1000 for performance (tags aggregation approximation)
